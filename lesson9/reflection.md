...
Анализируя эталонное решение я обнаружил несколько ошибок:
- Не были полностью проанализированы защитные сооружения за счет отсутствия структуры таблицы - приходилось самому представлять структуру
- Также было сложно анализировать информацию о погоде и также представлять структуру таблицы из-за ее отсутствия
- Неполностью проанализировал уязвимость зон крепости
- Неверно фильтровал данные (как в случае с `current_threat_level`)
- Неполностью были выведены данные в запросах и CTE
Сложность данного урока была в том, что необходимо было воспользоваться таблицами, структуры которых неизвестны, их можно было предположить только из описания, что я и делал. Думаю, что это было правильным вариантом, хоть и в решении у меня все равно были ошибки. Уверен, что при решении  проблем и задач в будущем, придется точно также окунаться в проблему, информация о которой не до конца известна и исходя из мысленных соображений и опыта - формировать решение.

Поражаюсь сложностью задач на данном курсе - и мне это нравится! Поскольку твой мозг ощущает дискомфорт, решая данные задачи, а значит - он прогрессирует, точно как в спортзале и мышцами. Уверен, что решение таких задач и прохождение курса в целом - пошло только на пользу. Также я подчерпнул как именно необходимо подходить к решению сложных и объемных задач. 
